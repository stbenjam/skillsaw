...
When working on skillsaw, do the best job you can.
...
skillsaw's Claude Code plugin ships a `PostToolUse` hook
(`hooks/hooks.json` → `hooks/post-tool-use.sh` → `skillsaw hook
post-tool-use`) that lints the file just edited and feeds violations back to
the agent.

- **The `hooks.json` `matcher` is the ONLY gate on which tools fire the
  hook** — it must enumerate every file-editing tool name
  (`Edit|Write|MultiEdit|Update`). Missing one (e.g. `Update`, the edit tool
  some Claude Code builds use) silently drops those edits.
- **The handler (`cli/_hook.py`) keys off `tool_input.file_path`, not tool
  name** — deliberately, so it can't drift out of sync with the matcher.
  Don't reintroduce a tool-name allowlist in Python.
- **The handler has NO allowlist of "files skillsaw lints"** — it lints the
  edited file's repository and reports only violations on that file, letting
  the lint tree's discovery be the single source of truth. Don't add a
  filename/suffix allowlist: it silently drifts (it once dropped
  `*.instructions.md`). If skillsaw doesn't discover the file, it produces no
  violations and the hook stays silent on its own.
- **The hook is fail-safe**: any malformed input, missing file, broken
  config, or internal error exits 0 silently. It exits 2 (surfacing stderr
  to the agent) only for violations at/above the repo's configured
  `fail-on`, and lints with the repo's own `.skillsaw.yaml` — no flag
  overrides.
- **`make update` propagates the plugin hook into `.claude/`, `.cursor/`,
  `.codex/`, and `.github/`** so skillsaw dogfoods it on itself across all
  assistants. Those generated copies are committed on purpose — don't
  hand-edit them; edit `hooks/` and regenerate.
