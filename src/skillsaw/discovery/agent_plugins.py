...
# does not match a URI merely mentioned in description text.
...
def declared_schema(path: Path, package_root: Path, kind: str) -> Optional[str]:
    ...
    if not error and isinstance(data, dict):
        ...
        if is_agent_plugin_schema(value, kind):
            return value

    content = read_text(resolved)
    if content and kind == "plugin" and _SCHEMA_PROPERTY_RE.search(content):
        match = re.search(
            rf'https://agent-plugins\.org/schemas/[^"/]+/{kind}\.schema\.json',
            content,
        )
...
def discover_agent_plugins(root: Path, *, forced: bool = False) -> List[Path]:
    ...
    for candidate in candidates:
        ...
        if (
            ...
            or not declares_agent_plugin(candidate)
        ):
            continue
