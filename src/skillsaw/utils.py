...
#: A compiler stamps its banner at the top of the file, on a line of its own.
_GENERATED_HEADER_LINES = 10
_BANNER_LINE = re.compile(r"^\s*(?:<!--.*-->|<!--.*|#|//)", re.DOTALL)


def has_generated_header(text: Optional[str]) -> bool:
    """Whether *text* is stamped as compiled output by a banner in its header.

    The strict counterpart to :func:`has_generated_marker`, for callers whose
    false positive is expensive rather than cheap. The lint tree uses this to
    drop APM's compiled Copilot file: being wrong there removes the file from
    every content, security and budget rule, so "the document mentions the
    phrase somewhere" is nowhere near enough evidence. An authored file
    reading "review code generated by APM CLI before merging" is prose about
    generated output, not generated output.

    Requiring a comment line in the header, rather than a position alone,
    is what separates the two: APM writes
    ``<!-- Generated by APM CLI from .apm/ primitives -->`` as line 1, and a
    sentence never looks like that. A generator that stamps its banner as
    plain prose simply is not recognised, and its file keeps being linted —
    the safe direction.
    """
    if not text:
        return False
    for line in text.split("\n", _GENERATED_HEADER_LINES)[:_GENERATED_HEADER_LINES]:
        if _BANNER_LINE.match(line) and _GENERATED_MARKER.search(line):
            return True
    return False
