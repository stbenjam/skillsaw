...
class TestClaudeMdAgentsImportRule:
    ...
    def test_agents_md_importing_claude_md_is_silent(self, temp_dir):
        """The reverse arrangement already has one source of truth. Telling
        CLAUDE.md to import AGENTS.md would create an import cycle."""
        (temp_dir / "CLAUDE.md").write_text(AGENTS_BODY)
        (temp_dir / "AGENTS.md").write_text(
            "# Agent Instructions\n\n@CLAUDE.md\n\nRead the file above in full before "
            "writing any code. Do NOT add rules here.\n"
        )
        assert self._check(temp_dir) == []

    def test_agents_md_pointer_stub_naming_claude_md_is_silent(self, temp_dir):
        (temp_dir / "CLAUDE.md").write_text(AGENTS_BODY)
        (temp_dir / "AGENTS.md").write_text(
            "# Agents\n\nThis file is a pointer, not a second copy. All guidelines are\n"
            "maintained in `CLAUDE.md` so there is exactly one set to keep current.\n"
        )
        assert self._check(temp_dir) == []
    ...
    def test_long_agents_md_that_merely_mentions_claude_md_is_still_reported(self, temp_dir):
        ...
        (temp_dir / "AGENTS.md").write_text(
            AGENTS_BODY
            + "".join(f"\nRule {index}: keep commits focused.\n" for index in range(12))
            + "\nClaude Code users also have a CLAUDE.md with extra notes.\n"
        )
        assert len(self._check(temp_dir)) == 1

    def test_auto_generated_banner_before_do_not_edit_is_ignored(self, temp_dir):
        """`AUTO-GENERATED — DO NOT EDIT. Built by ...` is the common banner
        order; the marker check must not require "generated by" first."""
        (temp_dir / "AGENTS.md").write_text(AGENTS_BODY)
        (temp_dir / "CLAUDE.md").write_text(
            "<!-- AUTO-GENERATED — DO NOT EDIT. Built by `ai update-md` from AGENTS-Source.md. -->\n"
            + AGENTS_BODY
        )
        assert self._check(temp_dir) == []
