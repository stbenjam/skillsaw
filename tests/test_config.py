...
def test_auto_with_formats_enabled_when_format_detected(temp_dir):
    ...
    config = LinterConfig.default()
    assert config.is_rule_enabled("cursor-mdc-valid", context, None, {HAS_CURSOR}) is True
...
def test_auto_with_formats_disabled_when_format_missing(temp_dir):
    ...
    context = RepositoryContext(temp_dir)
    config = LinterConfig.default()
    assert config.is_rule_enabled("cursor-mdc-valid", context, None, {HAS_CURSOR}) is False
...
def test_explicit_enabled_overrides_auto_detection(temp_dir):
    ...
    context = RepositoryContext(temp_dir)
    config = LinterConfig(rules={"cursor-mdc-valid": {"enabled": False}})
    assert config.is_rule_enabled("cursor-mdc-valid", context, None, {HAS_CURSOR}) is False

    config2 = LinterConfig(rules={"cursor-mdc-valid": {"enabled": True}})
    assert config2.is_rule_enabled("cursor-mdc-valid", context, None, {HAS_CURSOR}) is True


def test_format_specific_rules_default_to_auto(temp_dir):
    """Test that previously disabled format-specific rules now default to auto"""
    config = LinterConfig.default()
    auto_rules = [
