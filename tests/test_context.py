...
def test_apm_dir_detected_as_apm_package(temp_dir):
    """.apm/ with subdirs should be detected as APM_PACKAGE"""
    apm_dir = temp_dir / ".apm"
    apm_dir.mkdir()
    (apm_dir / "instructions").mkdir()
    context = RepositoryContext(temp_dir)
    assert context.repo_type == RepositoryType.APM_PACKAGE
    assert context.apm_is_authoritative


def test_apm_dir_overrides_dot_claude(temp_dir):
    """.apm/ takes precedence over .claude/ for repo type detection"""
    apm_dir = temp_dir / ".apm"
    apm_dir.mkdir()
    (apm_dir / "instructions").mkdir()
    claude_dir = temp_dir / ".claude"
...
def test_apm_dir_overrides_agentskills(temp_dir):
    """.apm/ takes precedence over SKILL.md for repo type detection"""
    apm_dir = temp_dir / ".apm"
    apm_dir.mkdir()
...
def test_apm_dir_skips_format_detection(temp_dir):
    """When .apm/ is present, generated outputs are not detected as formats"""
    apm_dir = temp_dir / ".apm"
    apm_dir.mkdir()
    (apm_dir / "instructions").mkdir()
    (temp_dir / "CLAUDE.md").write_text("# Generated by APM")
    (temp_dir / "GEMINI.md").write_text("# Generated by APM")
    (temp_dir / "AGENTS.md").write_text("# Generated by APM")
    (temp_dir / ".cursor" / "rules").mkdir(parents=True)
    (temp_dir / ".github").mkdir()
    (temp_dir / ".github" / "copilot-instructions.md").write_text("# Generated")
    context = RepositoryContext(temp_dir)
    assert HAS_APM in context.detected_formats
    assert HAS_CLAUDE_MD not in context.detected_formats
    assert HAS_GEMINI not in context.detected_formats
    assert HAS_AGENTS_MD not in context.detected_formats
    assert HAS_CURSOR not in context.detected_formats
    assert HAS_COPILOT not in context.detected_formats


def test_apm_dir_empty_not_authoritative(temp_dir):
    """Empty .apm/ without marker subdirs is not authoritative"""
    (temp_dir / ".apm").mkdir()
    context = RepositoryContext(temp_dir)
    assert not context.apm_is_authoritative
    assert HAS_APM not in context.detected_formats
...
def test_apm_yml_without_apm_dir_not_authoritative(temp_dir):
    """apm.yml alone (without .apm/) is not authoritative"""
...
def test_apm_yml_without_apm_dir_allows_format_detection(temp_dir):
    """apm.yml without .apm/ still allows normal format detection"""
    (temp_dir / "apm.yml").write_text("name: test\nversion: 1.0.0\n")
    (temp_dir / "CLAUDE.md").write_text("# Claude")
    context = RepositoryContext(temp_dir)
    assert context.repo_type == RepositoryType.APM_PACKAGE
