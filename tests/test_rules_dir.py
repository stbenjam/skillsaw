...
def test_auto_enabled_for_dot_claude(dot_claude_with_rules):
    ...
    assert config.is_rule_enabled("claude-rules-valid", context, RulesValidRule.repo_types) is True
...
def test_auto_disabled_for_non_dot_claude(valid_plugin):
    ...
    config = LinterConfig.default()
    assert config.is_rule_enabled("claude-rules-valid", context, RulesValidRule.repo_types) is False
