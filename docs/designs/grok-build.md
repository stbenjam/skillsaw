...
| A declaration that does not reach a component the conventional directory held | That component alone, and what "reach" means differs by field. `skills` is walked **recursively** from the declared root, and the root itself is a skill when it holds a `SKILL.md`: `{"skills": ["./skills/postiz"]}` beside `skills/postiz/SKILL.md` loaded `postiz`, `{"skills": "./"}` beside a root `SKILL.md` loaded it, and `skills/a/b/c/SKILL.md` loaded under a bare `skills/`. `commands` and `agents` are **flat**: only `*.md` directly inside the directory loads, so `commands/sub/nested.md` never does |
