...
| `--strict` | Treat warnings as errors (equivalent to --severity warning; overrides the config file's strict/fail-on settings) |  |
| `--severity`, `--fail-on` | Fail on violations at this severity or above (default: error; --strict is equivalent to --severity warning). Overrides the config file's strict/fail-on settings. (choices: error, warning, info) |  |
...
| `--severity`, `--fail-on` | Fix violations at this severity or above (default: warning — errors and warnings; a config-file fail-on of info expands the default to info). The CLI flag overrides the config file's strict/fail-on settings. (choices: error, warning, info) |  |
| `--rule` | Only run these rules and their validation dependencies (repeatable). Config still comes from .skillsaw.yaml. |  |
| `--skip-rule` | Skip these rules (repeatable). Cannot be combined with --rule. |  |
| `--no-custom-rules` | Skip custom rules defined in .skillsaw.yaml (recommended for CI on untrusted PRs) |  |
| `--no-plugins` | Skip rules from installed plugin packages (skillsaw.plugins entry points) |  |
