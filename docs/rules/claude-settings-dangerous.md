...
*Formerly known as `settings-dangerous`. The legacy name still works in configs, `--rule`/`--skip-rule`, suppression comments, and baselines.*
...
*Run `skillsaw explain claude-settings-dangerous` to see this documentation and the rule's effective configuration in your terminal.*
