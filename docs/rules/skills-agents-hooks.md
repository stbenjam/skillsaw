...
Validates skill/agent frontmatter and hook configuration. The `description-routing` rule checks when-to-use phrasing, first-person voice, and name restatements; each check is independently configurable. The security rules scan hooks in `hooks.json`, `.claude/settings*.json`, and skill/agent frontmatter (`hooks:` key) for supply-chain attack patterns (inspired by the [Shai-Hulud attack](https://safedep.io/mini-shai-hulud-strikes-again-314-npm-packages-compromised/)).
...
| [`description-routing`](description-routing.md) | Descriptions should tell the model when to route work to a skill, agent, or command | warning (auto) | - |
