...
- **Charge a cache entry once, at admission, and credit back the stored
  ...
  `sys.getsizeof`. An entry is also more than its value: the `Path` key,
  bucket, sub-key and dict slots run past 500 bytes, which is the whole cost
  for a repository of tiny files. Measure with `_entry_cost` at admission
