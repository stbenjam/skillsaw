...
- **Hooks get one subclass per host instead of conditional strictness.**
  ...
  loads it, and 1.0.13 publishes no observable for that path.
...
`repository_<ecosystem>.py` mixin when `test_module_layering` caps it
(Grok's are there); a `RepositoryType` member per packaging
claim, in `SKILL_REPO_TYPES` when its plugins carry skills (what turns the
`agentskill-*` rules on) and in `_TYPE_PRIORITY`, or `repo_type` reads
`unknown`; its marker directory in `PLUGIN_MARKER_DIR_NAMES`
(`discovery/detect.py`), so one walk finds a package's plugin or catalog —
never a second traversal; its config-file cluster in the
single plugin pass in `build_lint_tree` (through a contained helper);
...
declared per rule class. The exception is a rule keying on an explicit
`repo_types` list, like `content-description-routing`.
