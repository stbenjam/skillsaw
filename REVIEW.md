...
- Review **secrets and command execution**: flag `subprocess` with `shell=True` on untrusted input, and secrets written to logs or artifacts.
- Check **outbound network calls (T18)**: `content-broken-external-reference` is the *only* rule permitted to make them. Flag any other new network call, and require justification for widening this one — a rule that reaches the network must set `requires_network = True` (so `--no-network` gates it), stay `default_enabled = False`, confine destinations to public hosts, re-vet redirect targets, clamp every repo-supplied option, and be tested against a local `http.server` rather than the internet.
