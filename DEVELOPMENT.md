...
This runs 7 specialist reviewers (architecture, Python, security, QA, docs, ecosystem, palimpsest) as parallel sub-agents plus an arbiter. Reviewing a PR posts a structured verdict as a PR comment; reviewing the current local branch prints the verdict instead. Use `--serial` for cheaper inline execution.
