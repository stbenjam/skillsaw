...
| T8 | Supply chain compromise of the skillsaw PyPI package or its dependencies | supply_chain | pypi_supply_chain | developer_workstation, pypi_package | critical | rare | partially_mitigated | PyPI publish uses trusted publishing (OIDC) with Sigstore provenance attestations (`gh-action-pypi-publish`, `attestations: true`). CycloneDX SBOM published with each release covering all dependencies including optional extras (llm, vertexai, bedrock). Runtime dependencies pinned with hashes in `requirements/lock.txt`; optional extras pinned in `requirements/llm-lock.txt`. Both verified with `--require-hashes` during release builds and container image builds. Dependabot opens weekly PRs for lockfile updates. | None |
...
| ~~Pin dependency hashes in a lockfile~~ (done — `requirements/lock.txt` with `--require-hashes`, Dependabot for updates) | T8 | partial | M |
| ~~Publish an SBOM with releases~~ (done — CycloneDX SBOM attached to GitHub Releases) | T8 | partial | S |
| ~~Sign PyPI releases with Sigstore and publish provenance attestations~~ (done — PR #241) | T8 | partial | M |
