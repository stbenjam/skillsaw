...
- updated: 2026-08-29 — YAML parsing moved to libyaml (`CSafeLoader`) for speed. libyaml's composer recurses in C without a guard, so the depth bound is enforced ahead of it over the iterative event stream rather than on the constructed object; T5's controls and evidence updated accordingly. Documents libyaml rejects but PyYAML's own scanner accepts (JSON-style escaped surrogate pairs, `%YAML` directives naming an unimplemented version) are retried on the pure-Python loader, so the accepted-document set and the reported error marks are unchanged. T11 notes the new `FileCache` byte budget. Both readers now carry both bounds — the pre-compose event count and the post-parse graph measurement — because a shallow source of anchors can build a graph of any depth, which the event stream cannot see.
