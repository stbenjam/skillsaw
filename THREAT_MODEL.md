...
| T19 | Repo-controlled regex compilation: a Muse Code hooks file's `matcher` field is defined within the repository, and `muse-hooks-valid` compiles it as a regular expression during validation | remote_unauth | editor_tool_inputs | ci_gate_integrity | low | possible | mitigated | The repo-controlled pattern is compiled for syntax validation only and is never used to match input text, so it cannot drive match-time backtracking (ReDoS). It is, however, *scanned*: before compiling, the rule rewrites Rust-only atoms into their Python spellings, running two of skillsaw's own patterns over the matcher and walking it once. Four things bound that work — matchers longer than 1,000 characters are not compile-checked at all (and report nothing, since Muse imposes no length limit, so length is not a defect); the Unicode-class pattern's braced name is capped at 64 characters; the class-set-operator pass is a single linear walk that consumes escapes whole and never backtracks; and `re.compile` is wrapped in an exception guard (`re.error`, `RecursionError`, `OverflowError`) so a pathological pattern produces a violation rather than interrupting the lint run. | `muse-hooks-valid` |
