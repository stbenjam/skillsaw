...
| editor_tool_inputs | Cursor rules, commands, hooks and MCP; Copilot instructions, prompts and custom agents; OpenCode JSON/JSONC config, content and configured instruction paths/globs; Devin/Windsurf instructions, rules and skills; Muse Code's `.muse/hooks.json` (hook commands, and an event `matcher` compiled as a regex — never matched against input) and `.agents/memory/` (`MEMORY.md`, an index injected into context at session start, plus the topic files it lists by path, which an agent reads on demand — all of it prose reaching an agent's context window, a prompt-injection surface for the content security rules) | untrusted repo content → offline Markdown/JSON/JSONC/YAML parsing, path resolution, rule engine, regex compiler | repository_files, ci_gate_integrity |
...
| T19 | Repo-controlled regex compilation: a Muse Code hooks file's `matcher` field is entirely repository-controlled, unlike T13's operator-authored `.skillsaw.yaml` patterns, and `muse-hooks-valid` compiles it as a Python regex to validate the file | remote_unauth | editor_tool_inputs | ci_gate_integrity | low | possible | mitigated | The pattern is only compiled, never matched against input, so match-time catastrophic backtracking cannot be triggered; the compile call is wrapped so a pathological or invalid pattern is reported as a violation instead of crashing the rule. | `muse-hooks-valid` (this change) |
