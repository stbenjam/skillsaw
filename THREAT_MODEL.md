...
| T19 | Repo-controlled regex compilation: a Muse Code hooks file's `matcher` field is defined within the repository, and `muse-hooks-valid` compiles it as a regular expression during validation | remote_unauth | editor_tool_inputs | ci_gate_integrity | low | possible | mitigated | The pattern is only compiled for syntax validation and is never matched against input text, preventing match-time backtracking (ReDoS). The compilation call is wrapped with exception handling so invalid or complex patterns produce helpful violations rather than interrupting the lint run. | `muse-hooks-valid` |
