...
| T11 | Denial of service via resource exhaustion in discovery or parsing | remote_unauth | target_repo_files | developer_workstation | medium | possible | partially_mitigated | Recursive skill detection tracks resolved directories; HTML comments use a memoized linear scan; recursive YAML is caught before composition (see T5); known heavy directories are skipped. Both retention surfaces are bounded by byte budgets that charge each entry what it retains, so a repository of many small files or of astral-heavy text cannot grow either past its stated bound: cached reads by `FileCache.DEFAULT_BUDGET`, and the path-resolution memo by `paths._RESOLVE_CACHE_BUDGET_BYTES`, which holds manifest-supplied path strings and charges a path's components and the copies of its own string that a `Path` keeps alive. The memo never evicts — past its budget it stops accepting, which costs time and nothing else. Explicit whole-file size limits remain open, and are what make a ~150 KB pathological document cheap to deliver. | Symlink fan-out, unterminated-comment, recursive-YAML, and cache-budget tests |
...
- updated: 2026-08-29 — YAML parsing moved to libyaml (`CSafeLoader`) for speed. libyaml's composer recurses in C without a guard, so the depth bound is enforced ahead of it over the iterative event stream rather than on the constructed object; T5's controls and evidence updated accordingly. Documents libyaml rejects but PyYAML's own scanner accepts (JSON-style escaped surrogate pairs, `%YAML` directives naming an unimplemented version) are retried on the pure-Python loader, so the accepted-document set and the reported error marks are unchanged. T11 notes the new `FileCache` byte budget. Both readers now carry both bounds — the pre-compose event count and the post-parse graph measurement — because a shallow source of anchors can build a graph of any depth, which the event stream cannot see; the two round-trip write paths carry them too, through `roundtrip_yaml()`.
