...
- Evaluate whether `THREAT_MODEL.md` needs updating
    - Example: you added a new entry point (parser, file reader, network call, subprocess)
      Action: Add a row to §3 (Entry points) and consider whether a new §4 (Threats) row is needed
    - Example: you added or changed a security control (input validation, sandboxing, auth)
      Action: Update the `controls` column of the relevant §4 row
    - Example: you implemented a recommended mitigation from §8
      Action: Strike through the §8 row and update the §4 `status` column
