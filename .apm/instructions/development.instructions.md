...
- **Fold with `case_fold()`, never `str.lower()`, when gating an
  `re.IGNORECASE` pattern.** `lower()` leaves U+017F and U+212A alone
  where the engine matches them against `s` and `k`, so the gate drops
  documents the pattern matches.
- **Caches are bounded by memory, not by a count of entries.** A count low
