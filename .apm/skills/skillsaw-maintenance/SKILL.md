...
## Step 3: Address open issues

Open issues and recent comments are provided in the prompt. For each open issue:
- Read the issue description and any comments for context
- Check if an existing open PR already addresses it — if so, skip it
- Otherwise, create a separate PR that solves the issue
- Reference the issue in the PR description (e.g. "Closes #N")
- Follow the same PR conventions as Step 4 (tests, formatting, [Auto] prefix)

## Step 4: Implement spec gap fixes
